...
const PRESET_DATA: Record<string, { examName: string; daysLeft: number; subjects: SubjectItem[] }> =
  {
    ...
    cpa: {
      ...
      subjects: [
        ...
        {
          id: "4",
          name: "Taxation Laws & Regulations",
          weight: 15,
          status: "mastered",
          firstPass: true,
          qbankScore: 86,
    ...
    nclex: {
      ...
      subjects: [
        {
          id: "1",
          name: "Management of Care & Safety",
          weight: 20,
          status: "mastered",
          firstPass: true,
          qbankScore: 89,
...
export function HeroTrackerDemo() {
  ...
  const toggleStatus = (id: string) => {
    setSubjects((prev) =>
      prev.map((sub) => {
        ...
        const nextStatus =
          sub.status === "weak" ? "reviewing" : sub.status === "reviewing" ? "mastered" : "weak";
  ...
  const toggleFirstPass = (id: string) => {
    setSubjects((prev) =>
      prev.map((sub) => (sub.id === id ? { ...sub, firstPass: !sub.firstPass } : sub))
    );
  };
        ...
        <div className="flex flex-col gap-2 bg-muted/30 p-3 rounded-xl border border-border/50">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-muted-foreground font-mono">
              {Math.round((masteredCount / subjects.length) * 100)}% Complete
            </span>
          </div>
          <div className="w-full bg-muted h-2.5 rounded-full overflow-hidden flex">
            <div
              ...
              style={{
                width: `${(subjects.filter((s) => s.status === "mastered").length / subjects.length) * 100}%`,
              }}
            />
            <div
              className="bg-amber-400 transition-all duration-500 h-full"
              style={{
                width: `${(subjects.filter((s) => s.status === "reviewing").length / subjects.length) * 100}%`,
              }}
            />
            <div
              className="bg-rose-400 transition-all duration-500 h-full"
              style={{
                width: `${(subjects.filter((s) => s.status === "weak").length / subjects.length) * 100}%`,
              }}
            />
          </div>
        </div>
